...
function calcul_total_journee($start, $pause, $reprise, $fin){
...
function calcul_total_annee($annee,$id_horaire,$cjournee){
	...
	while ($row=$selectannee->fetch(PDO::FETCH_OBJ)) {
		if ($row->id_horaire != $id_horaire) {
			$recupdonnees[] = $row->total_journee;
		}
	}
...
$fin = $_POST['fin'];
$total_journee = $_POST['total_journee'];
...
$cjournee = calcul_total_journee($start, $pause, $reprise, $fin);
...
$updatehoraire = $bdd->prepare("UPDATE horaires SET start = :start, pause = :pause, reprise = :reprise,
	fin = :fin, total_journee = :total_journee, total_semaine = :total_semaine, total_mois = :total_mois, total_annee = :total_annee
