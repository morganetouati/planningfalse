...
if (!empty($id) && !empty($nom) && !empty($prenom) && !empty($start_contrat) && !empty($end_contrat) && !empty($heure_semaine) && !empty($salaire_brut) && !empty($salaire_net) && !empty($societe) && !empty($role_id)) {
	$membre = $bdd->prepare("UPDATE users SET nom = :nom, prenom = :prenom, start_contrat = :start_contrat, end_contrat = :end_contrat, heure_semaine = :heure_semaine, salaire_brut = :salaire_brut, salaire_net = :salaire_net, societe = :societe, role_id= :role_id WHERE id_users = :id");
